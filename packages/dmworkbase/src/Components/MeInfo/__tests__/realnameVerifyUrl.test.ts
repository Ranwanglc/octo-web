...
/**
 * YUJ-396 / YUJ-398 — Web 端「去认证」入口 URL 解析器单测。
 *
 ...
 *
 * YUJ-398 追加:return_to 必须 encodeURIComponent,且必须出现在拼好的 URL 尾部;
...
describe("resolveRealnameVerifyUrl", () => {
  it("returns the prod Aegis verify URL with encoded return_to when provider matches prod account_url", () => {
    const res = resolveRealnameVerifyUrl("xming", [aegisProdProvider], SAMPLE_RETURN_TO)
    expect(res).toEqual({
      ok: true,
  ...
  it("returns the test Aegis verify URL with encoded return_to (im-test)", () => {
    const res = resolveRealnameVerifyUrl("xming", [aegisTestProvider], SAMPLE_RETURN_TO)
    expect(res).toEqual({
      ok: true,
  ...
  it("encodeURIComponent escapes reserved chars in return_to (防 query pollution)", () => {
    // 模拟真实场景:return_to 里带 &、=、? —— 不 encode 直接拼会让 Aegis 把下游
    ...
    const res = resolveRealnameVerifyUrl("xming", [aegisProdProvider], malicious)
  ...
  it("returns local_account when loginProvider === 'local' (YUJ-396 behaviour preserved)", () => {
    const res = resolveRealnameVerifyUrl("local", [aegisProdProvider], SAMPLE_RETURN_TO)
    expect(res).toEqual({ ok: false, reason: "local_account" })
  })

  it("returns no_login_provider when loginProvider is an empty string", () => {
    const res = resolveRealnameVerifyUrl("", [aegisProdProvider], SAMPLE_RETURN_TO)
    expect(res).toEqual({ ok: false, reason: "no_login_provider" })
  })

  it("returns no_login_provider when loginProvider is undefined", () => {
    const res = resolveRealnameVerifyUrl(undefined, [aegisProdProvider], SAMPLE_RETURN_TO)
    expect(res).toEqual({ ok: false, reason: "no_login_provider" })
  })
  ...
  // YUJ-402 (Jerry R3 non-blocking suggestion promoted to strict contract):
  // 空 returnTo 不再静默拼 `return_to=`,直接 throw。调用方本地就能看到 bug,
  // 不等生产里用户回不来才发现。
  describe("[YUJ-402] empty returnTo throws", () => {
    it("throws when returnTo is an empty string", () => {
      expect(() =>
        resolveRealnameVerifyUrl("xming", [aegisProdProvider], ""),
    ...
    it("throws *before* provider branch checks (fail-fast on programming error)", () => {
      ...
      expect(() =>
        resolveRealnameVerifyUrl("local", [aegisProdProvider], ""),
      ).toThrow(/returnTo is required/)
    })
  })
})
