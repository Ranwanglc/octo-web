 ...
 *      到 window.location.href**(YUJ-398 P1 bug 的根因)
...
describe("MeInfoVM.startRealnameVerify — YUJ-398 window.open + return_to 合同", () => {
  ...
  it("xming provider → window.open(about:blank) 一次,拿到 mock window 后设置 opener=null + location.href 带 return_to(YUJ-402)", () => {
    // YUJ-402: 新实现先 open('about:blank') 拿窗口引用,再手动 opener=null + location.href 导航。
  ...
  it("window.open 返 null(弹窗拦截) → 仅 toast warning,**绝不**走 window.location.href fallback", () => {
    // YUJ-402: 新实现在 open 之后 return 掉,不再继续赋 location.href(mock window 根本没拿到)。
    // 这里单独验证 null → 唯一副作用是 toast warning。
    const openSpy = vi.fn().mockReturnValue(null)
    window.open = openSpy as unknown as typeof window.open

    // 监听 window.location.href 赋值 —— YUJ-398 明确禁止这条 fallback 链路。
  ...
  // YUJ-402:`opened.opener = null` setter 在部分沙箱下可能被 freeze。实现里用
  // try/catch 吞掉 — 这里显式 fixture 验证:即使 opener setter throw,也仍然要
  // 完成 location.href 导航,并且不对用户报错。
  it("[YUJ-402] opener setter 抛错时,仍然继续 location.href 导航 + 不 toast error", () => {
  ...
  it("loginProvider=local → Toast.error,不调 window.open(保留 YUJ-396 行为)", () => {
